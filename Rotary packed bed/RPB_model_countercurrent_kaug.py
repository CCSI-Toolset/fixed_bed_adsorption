...
def RPB_model(mode, parameters, gas_flow_direction=1):
    m = ConcreteModel()
    ...
    m.delH_3 = Var(initialize=parameters["delH_3"])
...
def full_model_creation(lean_temp_connection=True, configuration="co-current"):
    ...
    RPB.C1 = Param(initialize=2.562434e-12, mutable=True)
    RPB.hgx = Param(initialize=25 * 1e-3, mutable=True)
    RPB.delH_1 = Param(initialize=98.76, mutable=True)
    RPB.delH_2 = Param(initialize=77.11, mutable=True)
    ...
    RPB.con4 = Constraint(rule=hgx_con2)
    RPB.con5 = Constraint(rule=h1_con1)
    RPB.con6 = Constraint(rule=h1_con2)
    RPB.con7 = Constraint(rule=h2_con1)
...
def full_contactor_plotting(blk, save_option=False):
    ...
    # Adsorber Gas phase CO2 concentration
    C_CO2 = [[], [], [], [], []]
    k = 0
    for j in theta_nodes:
        for i in z:
            C_CO2[k].append(blk.ads.C["CO2", i, theta[j]]())
        k += 1

    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xlabel("Normalized Axial distance", fontsize=16)
    ax.set_ylabel("Gas phase CO$_{2}$ concentration, Adsorber", fontsize=12)
    # ax.set_ylim([0, 0.05])
    # ax.set_title('Adsorption gas phase CO$_{2}$')
    for i in range(len(theta_nodes)):
        ax.plot(
            z, C_CO2[i], "-o", label="theta=" + str(round(theta[theta_nodes[i]], 3))
        )
    ax.plot(z, [blk.ads.y_kz["CO2", j]() for j in z], "--", label="Averaged")
    ax.legend()

    if save_option:
        fig.savefig("CO2_concentration_ads.png", dpi=300)
    ...
    for j in theta_nodes:
        for i in z:
            C_CO2[k].append(blk.des.C["CO2", i, theta[j]]())
        k += 1

    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xlabel("Normalized Axial distance", fontsize=16)
    ax.set_ylabel("Gas phase CO$_{2}$ concentration, Desorber", fontsize=12)
    # ax.set_ylim([0, 0.05])
    # ax.set_title('Adsorption gas phase CO$_{2}$')
    for i in range(len(theta_nodes)):
        ax.plot(
    ...
    if save_option:
        fig.savefig("CO2_concentration_des.png", dpi=300)

    # Adsorber Gas phase N2 concentration
    C_N2 = [[], [], [], [], []]
    k = 0
    for j in theta_nodes:
        for i in z:
            C_N2[k].append(blk.ads.C["N2", i, theta[j]]())
        k += 1

    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xlabel("Normalized Axial distance", fontsize=16)
    ax.set_ylabel("Gas phase N$_{2}$ concentration, Adsorber", fontsize=12)
    # ax.set_ylim([0, 0.05])
    # ax.set_title('Adsorption gas phase CO$_{2}$')
    for i in range(len(theta_nodes)):
        ax.plot(
    ...
    if save_option:
        fig.savefig("N2_concentration_ads.png", dpi=300)
    ...
    for j in theta_nodes:
        for i in z:
            C_N2[k].append(blk.des.C["N2", i, theta[j]]())
        k += 1

    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xlabel("Normalized Axial distance", fontsize=16)
    ax.set_ylabel("Gas phase N$_{2}$ concentration, Desorber", fontsize=12)
    # ax.set_ylim([0, 0.05])
    # ax.set_title('Adsorption gas phase CO$_{2}$')
    for i in range(len(theta_nodes)):
        ax.plot(
    ...
    if save_option:
        fig.savefig("N2_concentration_des.png", dpi=300)
